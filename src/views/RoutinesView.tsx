...
export const RoutinesView = ({ store, setActiveTab, setModal }: any) => {
  ...
  return (
    <div className="space-y-6 pt-8 view-animate text-left">
      <header className="sticky top-0 z-50 px-2 py-4 -mx-2 bg-[#000000]/80 backdrop-blur-xl rounded-b-3xl">
        <h1 className="text-[2.5rem] font-black tracking-tight text-white pl-2">Schede</h1>
      </header>
        ...
        {routines.map((r: any, i: number) => (
          <div key={i} className="surface-card p-6 rounded-[2.2rem] space-y-5 shadow-lg">
                        ...
                        <div className="space-y-2">
                            {Object.entries(
                                ...
                                <div key={cat} className="break-words leading-snug">
                                    <span className="text-sky-400 uppercase text-[9px] tracking-widest font-black mr-2">{cat}</span>
            ...
            <div className="flex gap-3 pt-3">
              ...
              <button onClick={() => startWorkout(r)} className="flex-1 bg-sky-500 py-4 rounded-full font-bold uppercase text-[11px] text-white shadow-lg active:scale-[0.98]">Avvia</button>
            </div>
          </div>
        ))}
